...
def main() -> None:
    ...
    print_banner()
    print_color("Initializing orbfarmer...", Colors.CYAN)
        ...
        while True:
            try:
                ...
                if choice == '1':
                    ...
                    print_color(f"\n[*] Thanks for using orbfarmer!", Colors.CYAN, bold=True)
                    print_color(f"[*] Developed by {config.DEVELOPER}", Colors.GRAY)
                    print_color(f"[*] Improvements by {config.CONTRIBUTOR}", Colors.GRAY)
                ...
                print_color("[*] Thanks for using orbfarmer!\n", Colors.CYAN)
                break
    finally:
        faker.cleanup()
